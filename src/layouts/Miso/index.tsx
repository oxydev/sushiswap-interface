                ...
                <>
                    <Image
                        ...
                        src={cloudinaryLoader({
                            src: auction?.auctionDocuments?.desktopBanner,
                            width: 1280,
                            height: 196
                        })}
                        width={1280}
            ...
            return (
                <>
                    <Image
                        alt={`${auction?.auctionInfo?.tokenInfo.name} banner`}
                        src={cloudinaryLoader({
                            src: auction?.auctionDocuments?.mobileBanner,
                            width: 768,
                            height: 360
                        })}
                        width={768}
    ...
    return (
        <header
            ...
            className={classNames(
                'relative w-full bg-opacity-80 flex flex-col items-center shadow-md h-full',
                className
            )}
        >
            {background}
...
export const MisoBody: FC<MisoBodyProps> = ({ children, className, maxWidth = '7xl' }) => {
    return (
        <Main>
            <Container
                maxWidth={maxWidth}
                className={classNames('flex flex-col gap-10 py-10 px-5 lg:px-6 z-[1]', className)}
            >
                {children}
            </Container>
        </Main>
