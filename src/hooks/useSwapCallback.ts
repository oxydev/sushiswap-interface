...
function useBridgeCallArguments(
  ...
  return useMemo(() => {
    ...
    return swapMethods.map(parameters => ({ parameters, contract }))
  }, [account, chainId, library, recipient, trade])
}
...
export function useBridgeCallback(
  ...
  const swapCalls = useBridgeCallArguments(trade, recipientAddressOrName)
    ...
    return {
      ...
      callback: async function onSwap(): Promise<string> {
        const estimatedCalls: EstimatedSwapCall[] = await Promise.all(
          swapCalls.map(call => {
            const {
              parameters: { methodName, args, value, to },
              contract
            } = call
            const options = !value || isZero(value) ? {} : { value }
            if(contract){
        ...
        if (contract) {
          return contract[methodName](...args, {
            ...
            .then((response: any) => {
              ...
              const base = `Bridged ${inputToken} from  ${inputChain} to ${outputChain}`
              const withRecipient =
                recipient === account
                  ? base
                  : `${base} to ${
