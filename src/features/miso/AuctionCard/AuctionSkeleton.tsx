import React from 'react'

const AuctionSkeleton = () => {
    return (
        <div className="h-[443px] animate-pulse rounded bg-dark-900 pt-4 overflow-hidden transition-all shadow-md hover:translate-y-[-3px] hover:shadow-xl hover:shadow-pink/5">
            <div className="flex flex-col h-full gap-3">
                <div className="grid justify-between grid-cols-3 gap-2 px-5">
                    <div className="w-20 h-2 rounded bg-dark-800" />
                    <div />
                    <div className="w-20 h-5 rounded bg-dark-800" />
                </div>
                <div className="flex flex-col gap-2 px-5 pt-2">
                    <div className="w-20 h-2 rounded bg-dark-800" />
                    <div className="w-[124px] h-6 bg-dark-700 rounded" />
                </div>
                <div className="flex items-center justify-between px-5 py-3 bg-dark-800 ">
                    <div className="grid items-center grid-cols-4 gap-3">
                        <div className="rounded-full bg-dark-700 h-[18px] w-[18px]" />
                        <div className="h-4 col-span-3 rounded bg-dark-700" />
                    </div>
                    <div className="h-4 rounded bg-dark-700" />
                </div>
                <div className="flex justify-between gap-2 px-5">
                    <div className="flex flex-col w-full gap-2">
                        <div className="w-full h-1 rounded bg-dark-800" />
                        <div className="w-full h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-2">
                        <div className="h-1 rounded bg-dark-800" />
                        <div className="h-3 rounded bg-dark-700" />
                    </div>
                </div>
                <div className="flex h-full items-center px-5 h-[90px]">
                    <div className="h-1 rounded bg-dark-700 px-5 w-full transform rotate-[10deg]" />
                </div>
                <div className="grid flex-grow grid-cols-2 gap-3 px-5 py-4 bg-dark-800">
                    <div className="flex flex-col w-full gap-1">
                        <div className="w-full h-1 rounded bg-dark-800" />
                        <div className="w-full h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="h-1 rounded bg-dark-800" />
                        <div className="h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="w-full h-1 rounded bg-dark-800" />
                        <div className="w-full h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="h-1 rounded bg-dark-800" />
                        <div className="h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="w-full h-1 rounded bg-dark-800" />
                        <div className="w-full h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="h-1 rounded bg-dark-800" />
                        <div className="h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="w-full h-1 rounded bg-dark-800" />
                        <div className="w-full h-3 rounded bg-dark-700" />
                    </div>
                    <div className="flex flex-col w-full gap-1">
                        <div className="h-1 rounded bg-dark-800" />
                        <div className="h-3 rounded bg-dark-700" />
                    </div>
                </div>
            </div>
        </div>
    )
}

export default AuctionSkeleton
